...
void heapify(MaxHeap *m, int i)
{
    int largest = i, left = (2 * i) + 1, right = (2 * i) + 2;
    if (left < m->current_size && m->array[left] > m->array[largest])
        largest = left;
    if (right < m->current_size && m->array[right] > m->array[largest])
        largest = right;

    if (largest != i)
    {
        swap(&m->array[i], &m->array[largest]);
        heapify(m, largest);
    }
}

void removeElement(MaxHeap *m)
{
    int value;
    if (m->current_size == 0)
    {
...
void deleteAt(MaxHeap *m, int index)
{
    int value, j, parent;
    if (m->current_size == 0)
    {
...
void display(MaxHeap *m, int level, int index, char child)
{
    if (m->current_size == 0)
    {
        printf("Heap is empty\n");
...
int main()
{
    ...
    printf("Enter the size of the Max heap to be created: ");
    scanf("%d", &size);
    m = createMaxHeap(size);
    ...
    while (1)
    {
        ...
        {
        case 1:
            printf("Enter the element to be inserted into the max heap: ");
            scanf("%d", &value);
            insert(m, value);
            break;
        case 2:
        ...
        case 5:
            ...
            if (res == INT_MIN)
                ...
                printf("The largest element in the heap array is %d\n", res);
            break;
        default:
